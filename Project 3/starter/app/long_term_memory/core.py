...
class LongTermMemory:
    ...
    async def add_memory(self,
        ...
        self._container.create_item(body=item.to_dict())
    ...
    def get_memory(self, memory_id: str, session_id: str) -> Optional[MemoryItem]:
        ...
        try:
            # Read memory from Cosmos DB using item id and partition key (session_id)
            item_data = self._container.read_item(item=memory_id, partition_key=session_id)

            # Convert to MemoryItem
            mem = MemoryItem.from_dict(item_data)
            ...
            self._container.upsert_item(body=mem.to_dict())
